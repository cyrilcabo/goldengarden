...
const AboutGoals = () => {
	...
	const mappedGoals = goals.map((item, key) => {
		return <Grid item key={key} xs={12} sm={6} container direction="column" alignItems="center" className={classes.container}>
			<Grid item> 
	...
	return (
		<Grid item xs={12} className={classes.root} container alignItems="center" justify="center">
			<Grid item container xs={11} md={10} justify="space-between">
				{mappedGoals}
			</Grid>
		</Grid>
	);
